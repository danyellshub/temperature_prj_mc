---
title: "Cirque Meadows lapse rate analysis"
...
Lapse rate is often needed for calculating temperature gradients which is important when measuring water resources in mountain driven watersheds. 

--> Antonio's paper
--> 6.5°C/km in cascadian mountains paper
...
The mountain campus & Cirque Meadow Trail. 
High park fire in 2012
...
#### Making site data spatial and adding buffer for basemap
...
Collected data on Oct 13th. Read into R. Cleaned data. (No NAs). 
Applied linear models to
1) monthly
2) daily
3) hourly
4) night and day (diurnal)

Pulled data from Dark Sky
...
**Figure 8**: Hourly lapse rate in degrees Celsius per kilometer over the eleven sites. Most lapse rates are negative, though occosionally become positive indicating warmer air high in elevation. 
...
**Figure 9**: Interecepts of the hourly linear regression models. There are some values that are realistic, but many of these intercepts are so large this indicates this model cannot be applied to a large scale. 
...
**Figure 14**: Day and night lapse rates along with R^2^ values plotted through time of the eleven stations. The smaller the point, the smaller the R^2^ value, and this often happened closer to a lapse rate of 0 °C/km. The initial values while only a few of the stations were 'online' still had sizeable R^2^ values.
...
Overall trends, but more importantly
what to do next!
