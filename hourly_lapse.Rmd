...
## BM30 vs BM31, An Analysis

```{r, warning = FALSE}
merge1  <- merge_temp_location%>%
  arrange(date)

thirty_one <- merge_temp_location %>%
  filter(site %in% c("BM30","BM31"))

ggplot(thirty_one, aes(x=date, y=temp, color = site))+
  geom_point()

thirty_one_wide <- thirty_one %>%
  select(1:3)%>%
  spread(., key = 'site',
         value = 'temp')

ggplot(thirty_one_wide, aes(x=BM30, y=BM31)) +
  geom_point()+
  theme_few()+
  geom_abline()

ggscatter(thirty_one_wide,
          x = "BM30", 
          y = "BM31", add = "reg.line")+
  stat_cor(label.x = -8, label.y = 13) +
  stat_regline_equation(label.x = -8, label.y = 11) + 
  labs(x='BM30', y= 'BM31')+
  theme_linedraw()

difference <- thirty_one_wide %>%
  mutate(difference = BM31 - BM30)

ggplot(difference, aes(x = date, y= difference))+
  geom_point()

difference_xts  <- difference %>%
  select(1,4)%>%
  xts(., order.by = .$date)

dygraph(difference_xts)
